			    ...
			    int begin, int end)
{
  ...
  obstack_fgrow2 (oout, "const %s = OffsetArray([\n  %6d",
  ...
  /* DBM breaks if table > 9999 */
  obstack_fgrow1 (oout, "], 0:%4d)\n\n", len);
}
...
{
  /* DBM:: Changed to Julia syntax (and function rather than macro defintion) */
  obstack_sgrow (&table_obstack, "\
\n\
# YYTRANSLATE(YYLEX) -- Bison token number corresponding to YYLEX.\n");

  obstack_fgrow2 (&table_obstack,
      "YYTRANSLATE(x) = ((x) <= %d ? yytranslate[x] : %d)\
\n\
\n",
...
output_gram (void)
{
  ...
  /* DBM:: with "debug mode" alway on, get ride of conditional compile */
  /*
...
output_rule_data (void)
{
  ...
  */
  ...
  /*DBM::  obstack_sgrow (&table_obstack, "#endif\n\n");  */

  if (token_table_flag || no_parser_flag)
    {
      obstack_fgrow1 (&table_obstack, "const YYNTOKENS = %d\n", ntokens);
      obstack_fgrow1 (&table_obstack, "const YYNNTS = %d\n", nvars);
      obstack_fgrow1 (&table_obstack, "const YYNRULES = %d\n", nrules);
      obstack_fgrow1 (&table_obstack, "const YYNSTATES = %d\n", nstates);
      obstack_fgrow1 (&table_obstack, "const YYMAXUTOK = %d\n\n",
...
# YYTNAME[TOKEN_NUM] -- String name of the token TOKEN_NUM.\n");
      ...
      for (p = tags[i]; p && *p; p++)
	{
	  if (*p == '"' || *p == '\\')
	    ...
	    obstack_sgrow (&table_obstack, "\\b");
          /* Need to escape $ for Julia or it tries to interpolate */
          else if (*p == '$')
	    obstack_sgrow (&table_obstack, "\\$");
	  else if (*p < 040 || *p >= 0177)
  ...
  obstack_fgrow1 (&table_obstack, "], 0:%4d)\n\n", nsyms-1);
...
output_parser (void)
{
  ...
  */
  ...
  /* DBM if no skeleton, then open in memory beefalo (Julia) skeleton file */
  if (!skeleton) {
      fskel = fmemopen(bskel, strlen(bskel), "r");
  } else {
      assert (skeleton);
  ...
  while (1)
      ...
      enum line_type_e
	{
      ...
      /*DBM::  #'s are comments for Julia and Python so won't do here,
               but then #line directives are not supported       */
	...
	case sync_line:
	  if (!no_lines_flag)
	    obstack_fgrow2 (&table_obstack, "#line %d %s\n",
			    (int)line, quotearg_style (c_quoting_style, skeleton));
...
\n", debug_flag);
  */
  ...
  output_defines ();   /*DBM::  commented writes out.  Add here if necessary */
