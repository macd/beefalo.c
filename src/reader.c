	 ...
	 struct symbol_list *rule, int stack_offset)
{
  ...
  if (c == '$')
      ...
      while (i < n)
	{
      ...
      /* DBM:: */
      if ((n - stack_offset) != 0) {
          obstack_fgrow1 (oout, "yyls[yylsp%d]", n - stack_offset);
      } else { 
  	  obstack_sgrow (oout, "yyls[yylsp]");
      }
	     ...
	     symbol_list *rule, int stack_offset)
{
  ...
  if (c == '$')
	...
	type_name = get_type_name (0, rule);
      ...
      /* DBM:: Skip the type naming for yyval
      if (type_name) {
	...
	type_name = get_type_name (n, rule);
      ...
      /*DBM::  changed *yyvsp to yyvs[yyvsp] etc & added conditional here  */
      if ( (n - stack_offset) != 0) {
         obstack_fgrow1 (oout, "yyvs[yyvsp%d]", n - stack_offset);
      } else {
  	 obstack_sgrow (oout, "yyvs[yyvsp]");
      }
...
parse_union_decl (void)
{
  ...
  /* DBM:: */
  const char *prologue = "const YYSType = Union{";
  const char *epilogue = "}";
  ...
  while (c != EOF)
    {
      ...
      /* DBM:: we also skip writing the outer '{' and '}' */
      if (c != '/' && c != '{' && c != '}')
	{
	  obstack_1grow (&attrs_obstack, c);
	  if (defines_flag)
...
copy_action (symbol_list *rule, int stack_offset)
{
  ...
  */
  ...
  /* As a Bison extension, add the ending semicolon.  Since some Yacc
     ...
     missing semicolons.  */
  /* DBM:: ignore yacc flag for Julia */
  /*
  if (yacc_flag)
...
reader (void)
{
  ...
  /* Read the declaration section.  Copy %{ ... %} groups to
     ...
     etc. found there.  */
  obstack_fgrow3 (&table_obstack, "# %s, made from %s by GNU bison %s.  \n\n",
  ...
  /* DBM::  Always include location tracking
  if (defines_flag)
...
using OffsetArrays\n\
\n\
# needed for adding methods\n\
import Base: zero, zeros\n\
\n\
if !@isdefined(YYSType)\n\
    const YYSType = Any\n\
end\n\
\n\
mutable struct YYLType\n\
    first_line::Int\n\
    first_column::Int\n\
    last_line::Int\n\
    last_column::Int\n\
...
\n\
zero(YYLType) = YYLType(0, 0, 0, 0)\n\
zeros(YYLType, n) = [zero(YYLType) for i in 1:n]\n\
\n");
}
